...
const AddToCartModal: React.FC<AddToCartModalProps> = ({ open, onClose, menuItem, isLoading }) => {
  ...
  const handleAddonSelectionChange = useCallback((categoryId: string, option: AddonOption, type: 'checkbox' | 'radio') => {
    setSelectedAddons(prev => {
      ...
      const addonCategory = menuItem?.addon_categories?.find(cat => cat.id.toString() === categoryId);
                  ...
                  {cat.max_selections === 1 ? ( 
                    <RadioGroup
                      ...
                      onChange={(e) => {
                        ...
                        if (selectedOption) {
                          handleAddonSelectionChange(cat.id.toString(), selectedOption, 'radio');
                        }
                      }}
                    >
                        ...
                        <FormControlLabel
                          ...
                          control={
                            <Checkbox
                              ...
                              onChange={() => handleAddonSelectionChange(cat.id.toString(), option, 'checkbox')}
                            />
                          }
                          label={`${option.name} (R$ ${option.price.toFixed(2)})`}
                        />
